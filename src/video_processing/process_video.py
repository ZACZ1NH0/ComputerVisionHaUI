...
def process_video_file(video_path):
    ...
    if not cap.isOpened():
        logging.error(f"Không thể mở file video: {video_path}")
        return
    ...
    while True:
        ...
        # Có thể resize hoặc tăng tốc bằng cách skip frame:
        # frame = cv2.resize(frame, (width, height))
        # if frame_idx % 3 != 0: continue

        # Cắt ảnh mặt (nếu cần)
        result = detect_and_process_faces(frame, flag=4)  # bạn cần viết/đã có hàm này
