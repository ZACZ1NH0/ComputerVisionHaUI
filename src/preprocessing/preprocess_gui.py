...
def preprocess_images(person_name, parent_widget=None):
    person_name = person_name.strip()
    if not person_name:
        if parent_widget:
            QMessageBox.critical(parent_widget, "Lỗi", "Vui lòng nhập tên người dùng.")
        return
    ...
    if not os.path.exists(input_folder):
        if parent_widget:
            QMessageBox.critical(parent_widget, "Lỗi", f"Thư mục ảnh đầu vào không tồn tại:\n{input_folder}")
        return

    image_list = [f for f in os.listdir(input_folder) if f.lower().endswith(('.jpg', '.jpeg', '.png'))]

    if not image_list:
        if parent_widget:
            QMessageBox.information(parent_widget, "Thông báo", "Không có ảnh hợp lệ trong thư mục.")
        return
    ...
    if parent_widget:
        QMessageBox.information(parent_widget, "Hoàn tất", f"Đã xử lý xong {count} ảnh.")
